...
void test_nike_1(void)
{
  ...
  if (!ok)
  {
    ...
    for (unsigned long long i = 0; i < sizeof(secret_key); ++i)
...
secret_key priv_bob[KEYS];
public_key pub_bob[KEYS];
secret_key priv_alice[KEYS];
...
int main(int argc, char **argv)
{
  ...
  for (long long key = 0; key < KEYS; ++key)
  {
    ...
    csidh_secret(&priv_alice[key]);
    long long cycles1 = cpucycles();
    csidh_secret(&priv_bob[key]);
  ...
  for (long long key = 0; key < KEYS; ++key)
    ...
    {
      assert(memcmp(&priv_bob[key], &priv_bob[key2], sizeof(secret_key)));
