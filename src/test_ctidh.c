...
{
  fp P0Q2;
  fp_mul3(&P0Q2, &P[0], &Q[2]);
  fp P2Q0;
  fp_mul3(&P2Q0, &P[2], &Q[0]);
  if (!fp_isequal(&P0Q2, &P2Q0))
    return 0;
  fp P1Q2;
  fp_mul3(&P1Q2, &P[1], &Q[2]);
  fp P2Q1;
  fp_mul3(&P2Q1, &P[2], &Q[1]);
  if (!fp_isequal(&P1Q2, &P2Q1))
    return 0;
  return 1;
}
...
static void action_old(public_key *out, public_key const *in, secret_key const *priv)
{
  uintbig k[2];
  ...
  do
  {
    ...
    for (int64_t i = primes_num - 1; i >= 0; --i)
...
static void test_nike(void)
{
  ...
  secret_key priv_alice, priv_bob;
