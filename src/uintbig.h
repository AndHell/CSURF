...
extern const uintbig uintbig_p_cofactor;
extern const uintbig uintbig_1;
extern const uintbig uintbig_four_sqrt_p;
extern const uintbig uintbig_p_add1_div4;
...
{
  long long result = UINTBIG_LIMBS * 64;
  while (result > 0 && !uintbig_bit(x, result - 1))
    --result;
  return result;
}
...
static inline void uintbig_print(const uintbig *x)
{
  int i;
  printf("{");
  for (i = 0; i < UINTBIG_LIMBS; i++)
  {
    printf("0x%016lx,", (uint64_t)x->c[i]);
  }
  printf("\b}\n");
}
