...
void poly_mul_low(fp *c, long long clen, const fp *a, long long alen, const fp *b, long long blen)
{
  ...
  { /* XXX: tune this */
    ...
    fp a01[a0len];
    for (long long i = 0; i < a1len; ++i)
      fp_add3(&a01[i], &a0[i], &a1[i]);
    if (a1len < a0len)
      a01[a1len] = a0[a1len];

    fp b01[b0len];
    for (long long i = 0; i < b1len; ++i)
      fp_add3(&b01[i], &b0[i], &b1[i]);
    if (b1len < b0len)
      b01[b1len] = b0[b1len];
    ...
    for (long long i = 0; i < c0len; ++i)
      c[2 * i] = c0[i];
    for (long long i = 0; i < c01len; ++i)
      c[2 * i + 1] = c01[i];
    for (long long i = 0; i < c1len - 1; ++i)
      fp_add2(&c[2 * i + 2], &c1[i]);
    if (2 * c1len < clen)
      fp_add2(&c[2 * c1len], &c1[c1len - 1]);
...
void poly_mul_selfreciprocal(fp *c, const fp *a, long long alen, const fp *b, long long blen)
{
  ...
  if (alen == blen && (alen & 1))
  {
    ...
    for (long long i = 0; i < len0 + len0 - 1; ++i)
      c[2 * i] = c0[i];
    for (long long i = 0; i < len0 + len0 - 2; ++i)
      c[2 * i + 1] = c01[i];
    for (long long i = 0; i < len1 + len1 - 1; ++i)
      fp_add2(&c[2 * i + 2], &c1[i]);
    return;
  ...
  if (alen == blen && !(alen & 1))
  {
    ...
    for (long long i = 0; i < alen + alen - 1; ++i)
      c[i] = fp_0;
...
void poly_mul_mid(fp *c, long long cstart, long long clen, const fp *a, long long alen, const fp *b, long long blen)
{
  ...
  if (clen == 2)
  {
    ...
    if (cstart == 3 && alen >= 5 && blen == 4)
    {
      ...
      fp_sub3(&b23, &b[2], &b[3]);
      fp a1b23;
      fp_mul3(&a1b23, &a[1], &b23);
      fp a3b01;
      fp_mul3(&a3b01, &a[3], &b01);
      fp delta;
      fp_add3(&delta, &a1b23, &a3b01);
      fp a01;
      fp_add3(&a01, &a[0], &a[1]);
      fp a12;
      fp_add3(&a12, &a[1], &a[2]);
  ...
  if (clen == 4 && cstart == 5 && blen == 6 && alen >= 9)
  {
    ...
    for (long long i = 0; i < 6; ++i)
      fp_add3(&a01[i], &a[i], &a[i + 3]);

    fp b01[3];
    for (long long i = 0; i < 3; ++i)
      fp_sub3(&b01[i], &b[i], &b[i + 3]);
  ...
  if ((clen & 1) && cstart == clen && blen == clen + 1 && alen >= cstart + clen)
  {
    ...
    for (long long i = 0; i < 3 * split - 2; ++i)
      fp_add3(&a01[i], &a[i], &a[i + split]);

    fp b01[split];
    assert(2 * split == blen);
    for (long long i = 0; i < split; ++i)
      fp_sub3(&b01[i], &b[i], &b[i + split]);
    ...
    for (long long i = 0; i < split; ++i)
      fp_add2(&c[i], &delta[i]);
    for (long long i = 0; i < split - 1; ++i)
      fp_sub2(&c[i + split], &delta[i]);
    return;
  }
  ...
  if (!(clen & 1) && cstart == clen - 1 && blen == clen && alen >= cstart + clen)
  {
    ...
    fp a01[3 * split - 1];
    for (long long i = 0; i < 3 * split - 1; ++i)
      fp_add3(&a01[i], &a[i], &a[i + split]);
...
long long poly_multieval_unscaled_precomputesize(long long n, long long flen)
{
  ...
  long long m = n / 2;
  if (flen <= n)
    return poly_multieval_unscaled_precomputesize(m, flen) + poly_multieval_unscaled_precomputesize(n - m, flen);
  if (n == 2)
    return poly_pseudoremainder_precomputesize(n, flen) + poly_multieval_unscaled_precomputesize(1, n) + poly_multieval_unscaled_precomputesize(1, n);
  if (n == 3)
    return poly_pseudoremainder_precomputesize(n, flen) + poly_multieval_unscaled_precomputesize(2, n) + poly_multieval_unscaled_precomputesize(1, n);
  return poly_pseudoremainder_precomputesize(n, flen) + poly_multieval_unscaled_precomputesize(m, n) + poly_multieval_unscaled_precomputesize(n - m, n);
}
